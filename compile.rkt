...
(require "ast.rkt" "types.rkt" "lambdas.rkt" "fv.rkt" "compile-ops.rkt" "parse.rkt" "unload-bits-asm.rkt" a86 a86/ast)
  ...
  (match p
    [(Prog ds e)
     (prog (externs)
           ...
           (Mov r11 1)              ; Vector size
           (compile-e e (reverse (define-ids ds)) #f)
           (write-vector)           ; add single entry into vector
...
(define (write-vector)
  (let ((end (gensym 'end)))
    (seq (Cmp r11 1)
         (Jne end)
         (Mov (Offset rbx 0) r11) ; write size of vector
...
(define (compile-e e c t?)
  (match e
    ...
    [(Values vs)        (compile-values vs c t?)]
    ...
    [(Let-Values xs e1 e2)
     (compile-let-values xs e1 e2 c t?)]
...
; xs = '(#s(Var x) #s(Var y) #s(Var z))
; vs = '#s(Values (#s(Int 1) #s(Int 2) #s(Int 3)))
; e = '#s(Prim2 + #s(Var x) #s(Prim2 + #s(Var y) #s(Var z)))

(define (compile-let-values xs e1 e2 c t?)
  (seq (compile-e e1 c t?)
       (Mov r8 (length xs))
       (Cmp r8 r11)
       (Jne 'raise_error_align)
       (push-values 0 (length xs))
       (Mov r11 1)
       (compile-e e2 (append xs c) t?)
       (Add rsp (* (length xs) 8))))

(define (compile-values vs c t?)
  (if (eq? (length vs) 1)
    (seq (Mov r11 1)
         (compile-e (car vs) c t?))
    (seq (Mov r11 (length vs))
         (Mov (Offset rbx 0) r11)
         (compile-mult-return-values 8 vs c t?)
         (Mov rax rbx))))

(define (compile-mult-return-values iter vs c t?)
  (match vs
    ('() (seq))
    ((cons val rest)
     (seq  (compile-e val '() #f)
           (Mov (Offset rbx iter) rax)
           (compile-mult-return-values (+ 8 iter) rest c t?)))))

(define (push-values iter vals)
  (if (< iter vals)
      (seq (Mov r12 (Offset rax (* 8 (add1 iter))))
           (Push r12)
           (push-values (add1 iter) vals))
      (seq)))
...
;(tell '((let-values (((x y z) (values 1 2 3))) (+ x (+ y z)))))
