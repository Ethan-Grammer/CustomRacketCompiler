...
(define (parse-e s)
  (match s
    ...
    [(list 'let-values (list (list xs e1)) e2)
     (Let-Values xs (parse-e e1) (parse-e e2))]
    ...
    [(cons (? values? x) xs)
     (Values (parse-list xs))]
    [(cons e es)
     (App (parse-e e) (map parse-e es))]
    [_ (error "Parse error" s)]))

(define (parse-list ls)
  (map parse-e ls))
...
;; (parse '((let-values ((x y z) (values 1 2 3)) (+ x (+ y z)))))